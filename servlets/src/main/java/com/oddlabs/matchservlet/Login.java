...
public final class Login extends HttpServlet {
	...
	private void createUser(String reg_key, String username, String password_digest, String email) throws SQLException {
		...
		try {
			...
			int min_username_length = DBInterface.getIntSetting(connection, "min_username_length");
			if (username.length() < min_username_length)
				throw new SQLException("USERNAME_ERROR_TOO_SHORT");

			int max_username_length = DBInterface.getIntSetting(connection, "max_username_length");
			if (username.length() > max_username_length)
				throw new SQLException("USERNAME_ERROR_TOO_LONG");

			String allowed_chars = DBInterface.getSetting(connection, "allowed_chars");
			...
			int too_many = (Integer) DBInterface.executeQuery(connection, new GetFirstIntQuery(),
					"SELECT COUNT(*) FROM match_user WHERE reg_key = ?", reg_key);
			if (too_many > 0)
				throw new SQLException("USERNAME_ERROR_TOO_MANY");
			int existing = (Integer) DBInterface.executeQuery(connection, new GetFirstIntQuery(),
					"SELECT COUNT(*) FROM match_user WHERE username = ?", username);
			if (existing > 0)
				throw new SQLException("USERNAME_ERROR_ALREADY_EXISTS");
			int num_rows = DBInterface.executeUpdate(connection,
