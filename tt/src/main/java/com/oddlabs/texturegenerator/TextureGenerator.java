...
public final strictfp class TextureGenerator {
	...
	private final static int LOW_DETAIL_MIPMAP_CUTOFF = Globals.NO_MIPMAP_CUTOFF;
	...
	public static void main(String[] args) throws LWJGLException {
		if (args.length != 1) {
			throw new RuntimeException("Pass directory in the parameters");
		}
	...
	private void generateLowDetailTexture(OffscreenRenderer buffer, LowDetailModel[] models, String dest) {
		...
		do {
			drawBillboardsToBuffer(models, this, indices, buffer, Globals.COMPRESSED_RGBA_FORMAT, LOW_DETAIL_MIPMAP_CUTOFF, 0);
...
//			GL11.glClear(GL11.GL_COLOR_BUFFER_BIT | GL11.GL_DEPTH_BUFFER_BIT);
	...
	private final float[] getModelClearColor() {
		return trunks[0].getClearColor();
	}

	private final void renderModel(int mode, int tex_index) {
