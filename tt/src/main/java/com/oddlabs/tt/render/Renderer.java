...
import static com.oddlabs.util.Utils.LOG_FILES;
...
//		GL11.glClearColor(1f, 0f, 1f, 0f);
	...
	private static boolean readOrSetPreference(String key, boolean value) {
		return Boolean.valueOf(readOrSetPreference(key, "" + value));
	}

	private static int readOrSetPreference(String key, int value) {
		try {
			return Integer.parseInt(readOrSetPreference(key, "" + value));
		...
		try {
			if (!GLContext.getCapabilities().OpenGL13) {
				if (!GLContext.getCapabilities().GL_ARB_multitexture) {
					throw new LWJGLException("Neither OpenGL 1.3 nor GL_ARB_multitexture is supported, one of which is required for the game to run. " +
							"Please upgrade your video drivers and/or video card.");
				}
